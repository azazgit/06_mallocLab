// move the one from macos to here. 
// then test the tracefiles with c in them.
// check if ./mdriver -f tracefiles [with c files] works.
/*
...
typedef struct {
    enum {ALLOC, FREE, HEAPCHECK} type; /* type of request */
	...
	size = trace->ops[i].size;
	  ...
	  for(j=0;j<heapsize;j++) {
	    ...
	    if (trace->block_sizes[request_id] != payload) { 
	      sprintf(msg,"Heapcheck : Block %d has an incorrect block size of %d, expecting %d",request_id,payload,trace->block_sizes[request_id]);
	      malloc_error(tracenum,i,msg);
	      expected_blocks(trace);
	      outputted_blocks(heap,heapsize);
	      return 0;
